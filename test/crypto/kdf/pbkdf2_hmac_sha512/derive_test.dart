...
GrillTestResult hashingTest([int? iterations]) {
  return group('PBKDF2 HMAC SHA512 Derive', () {
    ...
    for (final testVector in testVectors) {
      test(testVector['name'], () {
        ...
        try {
          ...
          final actualHash = pbkdf2HMAC.deriveKeySHA512(password, salt, iterations, keyLength);
  ...
  return group('PBKDF2 HMAC SHA512 Derive', () {
    ...
    for (final testVector in testVectors) {
      ...
      testBenchmark(testVector['name'], () {
        pbkdf2HMAC.deriveKeySHA512(password, salt, iterations, keyLength);
      }, iterations: iterations);
    }
  });
}
